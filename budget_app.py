...
if not os.path.exists(DATA_FILE):
    ...
    df = pd.DataFrame({
        ...
        "Categoria": ["Alimentari"] * 5 + ["Bollette"] * 2 + ["Svago"] * 3,
        "Descrizione": ["Spesa settimanale"] * 5 + ["Luce", "Gas"] + ["Cinema", "Cena", "Bar"],
        "Importo": [-45, -50, -32, -48, -55, -120, -80, -25, -30, -15],
    })
...
# --- INPUT: Aggiungi nuova spesa/entrata ---
...
col1.metric("💰 Saldo Attuale", f"€ {saldo_attuale:,.2f}")
...
# Prendo gli ultimi 30 giorni per calcolare la spesa media giornaliera
...
# Consumo netto giornaliero (quanto perdo al giorno in media)
...
if burn_rate_giornaliero > 0:
    ...
    for i in range(13):
        ...
        saldo_futuro -= burn_rate_giornaliero * 30  # Sottraggo un mese
...
# --- BOTTONE PER RESETTARE I DATI (se vuoi ricominciare) ---
